        ...
        try {
            ...
            try {
                ...
                while (console.hasNextLine()) {
                    try {
                        Scanner scannerWord = new Scanner(console.nextLine());
                        try {
                            long[] numLine = new long[50];
                            int inx = 0;
                            while (scannerWord.hasNextNumber()) {
                                if (inx == numLine.length - 1) {
                                    numLine = Arrays.copyOf(numLine, numLine.length * 2);
                                }
                                String num = scannerWord.nextNumber();
                                long decimal = Long.parseLong(num, 8);
                                if (decimal % 2 != 0) {
                                    numLine[inx++] = Long.parseLong(num);
                                }
                            }
