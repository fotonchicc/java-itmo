...
public class Divide<T> extends BinaryOperations<T> {
    ...
    public Divide(GenericExpressions<T> firstTerm, GenericExpressions<T> secondTerm, GenericOperations<T> operation) {
        super(firstTerm, secondTerm, operation);
    }

    @Override
    protected T countRes(T firstTerm, T secondTerm) {
        return operation.divide(firstTerm, secondTerm);
    }
}
